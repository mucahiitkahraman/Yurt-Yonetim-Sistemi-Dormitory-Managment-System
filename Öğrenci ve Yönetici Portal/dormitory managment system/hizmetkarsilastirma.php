...
$query = "SELECT * FROM yurtlar WHERE yurt_ID IN (?, ?)";
$stmt = $pdo->prepare($query);
$stmt->execute([$dorm1Id, $dorm2Id]);
$dorms = $stmt->fetchAll();
?>
...
<head>
    ...
    .custom-table .oda-tipi {
        background-color: #FFE0B2; 
    }

    .custom-table .fiyatlar {
...
<table>
    ...
    <tr>
        ...
        <?php foreach ($dorms as $dorm) { ?>
            <td><?php echo $dorm['adres']; ?></td>
        <?php } ?>
    </tr>
    <tr>
        <td><?php echo $lang['dorm_capacity']; ?></td>
        <?php foreach ($dorms as $dorm) { ?>
            <td><?php echo $dorm['yurtKapasitesi']; ?></td>
        <?php } ?>
    </tr>
    <tr class="room-type">
        <td><?php echo $lang['room_type']; ?></td>
        <?php foreach ($dorms as $dorm) { ?>
            <td><?php echo $dorm['odaTipi']; ?></td>
        <?php } ?>
    </tr>
    <tr>
        <td><?php echo $lang['number_of_rooms']; ?></td>
        <?php foreach ($dorms as $dorm) { ?>
            <td><?php echo $dorm['odaSayisi']; ?></td>
        <?php } ?>
    </tr>
    <tr class="prices">
        <td><?php echo $lang['prices']; ?></td>
        <?php foreach ($dorms as $dorm) { ?>
            <td><?php echo $dorm['fiyatlar']; ?></td>
        <?php } ?>
    </tr>
    <tr>
        <td><?php echo $lang['past_prices']; ?></td>
        <?php foreach ($dorms as $dorm) { ?>
            <td><?php echo $dorm['gecmisFiyatlar']; ?></td>
        <?php } ?>
    </tr>
    <tr class="location">
        <td><?php echo $lang['location']; ?></td>
        <?php foreach ($dorms as $dorm) { ?>
            <td><?php echo $dorm['konum']; ?></td>
        <?php } ?>
    </tr>
    <tr class="service">
        <td><?php echo $lang['service']; ?></td>
        <?php foreach ($dorms as $dorm) { ?>
            <td><?php echo $dorm['hizmet']; ?></td>
        <?php } ?>
    </tr>
</table>
</div>
